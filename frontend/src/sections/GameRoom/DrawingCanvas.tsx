...
export const DrawingCanvas: React.FC<DrawingCanvasProps> = ({
  ...
  useEffect(() => {
    if (clearButtonRef.current && isDrawing) {
      gsap.fromTo(clearButtonRef.current,
  ...
  }, [currentTool, currentColor, brushSize, onDrawingChange]);
  ...
  const handleMouseDown = useCallback((event: React.MouseEvent) => {
    ...
    if (canvasRef.current) {
      ...
      ripple.className = 'absolute w-4 h-4 bg-blue-400 rounded-full pointer-events-none';
      ripple.style.left = `${event.nativeEvent.offsetX - 8}px`;
      ripple.style.top = `${event.nativeEvent.offsetY - 8}px`;
      canvasRef.current.parentElement?.appendChild(ripple);
      ...
      <div className="relative">
        <canvas
          ...
          className={`w-full h-auto border-2 border-gray-300 rounded-lg transition-all duration-300 ${
            isDrawing ? 'cursor-crosshair border-blue-300' : 'cursor-not-allowed'
          }`}
        ...
        >
          <div className="text-center">
            ...
            <p className="text-gray-600 font-medium">Waiting for your turn to draw...</p>
          </div>
        </div>
      </div>
    </div>
